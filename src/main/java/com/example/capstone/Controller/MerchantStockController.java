...
public class MerchantStockController {
    ...
    public ResponseEntity getMerchantStock() {
        ...
        return ResponseEntity.status(200).body("get merchantstock" + merchantStocks);
    }
    ...
    public ResponseEntity addMerchantStock(@Valid @RequestBody MerchantStock merchantStock, Errors errors) {
        ...
        return ResponseEntity.status(200).body("merchantstock" + merchantStock);
    }
    @PutMapping("/update/{id}")
    public ResponseEntity udpateMerchantStock(@PathVariable int id,@Valid @RequestBody MerchantStock merchantStock, Errors errors) {
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
